...
export const getBestTrainingPlayer = (training: Training) => {
  const combinedPointsForPlayers = training?.scores?.reduce((acc, score) => {
  ...
  const bestPlayer = training?.players.find(
